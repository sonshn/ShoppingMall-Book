...
router.post('/cart', function(req,res,next){
  ...
  else if(req.session.division!="customer")
  {
    ...
    console.log("book num : " + book_num);
    var sqlselect="select cart_num from cart where cust_id=? and book_num=?";
    pool.getConnection(function(err,connection){
        connection.query(sqlselect, [userid, book_num], function(err,rows){
            if(err) console.error("err : "+err);
            console.log("rows : " + JSON.stringify(rows[0]));
            if(rows[0]!=undefined) //장바구니에 이미 해당 아이디로 해당 책이 있는 경우
            {
              console.log("이미 장바구니에 있음");
              //res.send('<script type="text/javascript">alert("이미 장바구니에 추가된 도서입니다");location.href="/cart";</script>');
              //res.send('<script type="text/javascript">alert("구매자가 아닙니다.");hitory.back();</script>');
              //connection.release();
